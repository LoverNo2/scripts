...
图标识别在 1440 基准分辨率上进行：窗口宽度变化时（见 `env.py` 的 `view_width`），脚本先把截图缩小回 1440 基准再与原始模板匹配，模板无需为不同窗口宽度重新裁剪。
...
- `click_img` 匹配阈值固定为 0.85；若经常匹配不到，运行 `python debug_match.py` 查看每个模板的实际匹配分数，再决定调整模板或阈值。
